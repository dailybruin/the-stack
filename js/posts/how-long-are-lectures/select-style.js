...
names.forEach(item => {
  $(item.placeholder).on('click', function (ev) {
   ...
   $(`${item.list} a`).on('click', function (ev) {
     ...
     $(item.placeholder).text( $(this).text() ).css('opacity', '1');

     var option = $(item.list).find('li').eq(index)[0].attributes[0].nodeValue
     ...
     $(item.list).find('li').eq(index).prependTo(item.list);
     $(item.list).toggle();
