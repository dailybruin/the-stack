...
var baseLayer = L.tileLayer(
  'https://api.mapbox.com/styles/v1/mapbox/light-v9/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiamVycnlsaW5ldyIsImEiOiJjajIwc2E1YXkwMmt6MzNuMXZnaWRjb2lhIn0.u0AHZNQd8_8hFmpaVvI1nQ',
  {
    attribution:
...
for (let j = 0; j < layer_lists.length; j++) {
  ...
  for (let i = 0; i < layer_lists[j].length; i++) {
    ...
    var popup = name.concat(
      str1,
      "<span class='description'>" + layer_lists[j][i].status + '</span'
    );
    if (layer_lists[j][i].open == true)
      business_list.push(
        L.marker(L.latLng(lat, lon), { icon: greenIcon }).bindPopup(popup)
      );
    else
      business_list.push(
        L.marker(L.latLng(lat, lon), { icon: blackIcon }).bindPopup(popup)
      );
  }
